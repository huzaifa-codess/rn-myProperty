...
const PropertyScreen = () => {
    ...
    const {
        expanded,
        nearestPlacesChangableList,
        showSnackBar,
        isInWishList,
    } = state;

    const { propertyImage, propertyName, propertyAddress, propertyAmount } = useLocalSearchParams();
        ...
        return (
            <View style={styles.ownerInfoContentStyle}>
                <View style={{
                    ...
                    <TouchableOpacity
                        activeOpacity={0.9}
                        onPress={() => navigation.push('message/messageScreen', { name: 'John Smith' })}
            ...
            <View>
                {nearestPlacesChangableList.map((item) => (
                    <View key={item.id} style={{ marginHorizontal: Sizes.fixPadding * 2.0 }}>
                        <Collapse
                            onToggle={(isExpanded) => handleNearestPlacesUpdate({ id: item.id, isExpanded })}
                        >
                            <CollapseHeader>
                                <View
                                    style={{
    ...
    function photos() {
        const renderItem = ({ item }) => (
            <TouchableOpacity
                ...
                onPress={() => navigation.push('imageFullView/imageFullViewScreen', { propertyImage: item.photo, })}
            >
                <Image
                    source={item.photo}
